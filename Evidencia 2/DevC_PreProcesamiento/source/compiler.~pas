...
type
  ...
  TCompiler = class
   ...
   public
    ...
    procedure Preprocesar(SingleFile: string = ''); virtual;
...
begin
  ...
  {WriteLn(F,'main.i: main.cpp');
  WriteLn(F, #9 + '$(CPP) -E main.cpp -o main.i');
  WriteLn(F);}
  WriteLn(F,'main.i: main.c');
  WriteLn(F, #9 + '$(CC) -E main.c -o main.i');
  WriteLn(F);
...
var
 ...
 ShortPath: string;
 ResIncludes: String;
 tfile, ofile, tfile1, ofile1,ResFiles, tmp: string;
  ...
  begin
      ...
      begin
          ...
          begin
            ...
            ofile1 := IncludeTrailingPathDelimiter(fProject.Options.ObjectOutput)+ExtractFileName(fProject.Units[i].FileName);
            ofile1 := GenMakePath(ExtractRelativePath(fProject.FileName, ChangeFileExt(ofile1, OBJ1_EXT)));
         ...
         {****************************************************************************}
          ...
          begin
              ...
              if fProject.Units[i].CompileCpp then
                ...
                writeln(F, #9 + '$(CC) -S ' + GenMakePath(tfile) +
                  ...
                  writeln(F, GenMakePath2(ofile1) + ':' + GenMakePath2(tfile1));
              if fProject.Units[i].CompileCpp then
                writeln(F, #9 + '$(CPP) -S ' + GenMakePath(tfile1) +
                  ' -i nul $(CXXFLAGS)')
              else
                writeln(F, #9 + '$(CC) -S ' + GenMakePath(tfile1) +
                  ' -i nul $(CFLAGS)');
          ...
          begin
              ...
              begin
                if fProject.Units[i].CompileCpp then
                  writeln(F, #9 + '$(CPP) -c ' + GenMakePath(tfile1) +
                    ' -i ' + ofile1 + ' $(CXXFLAGS)')
                else
                  writeln(F, #9 + '$(CC) -c ' + GenMakePath(tfile1) +
                    ' -i ' + ofile1 + ' $(CFLAGS)');
              end;
...
{***************************************************************************}
procedure TCompiler.Preprocesar(SingleFile: string);
resourcestring
 cCmdLine = '%s "%s" -i "%s" %s %s %s';
 cMakeLine = '%s -f "%s" all';
 cSingleFileMakeLine = '%s -f "%s" %s';
 cMake = ' make';
 cDots = '...';
   ...
   begin
     ...
     if SingleFile<>'' then begin
        ...
        begin
          if not DirectoryExists(fProject.Options.ObjectOutput) then
            MkDir(fProject.Options.ObjectOutput);
          ofile1 := IncludeTrailingPathDelimiter(fProject.Options.ObjectOutput)+ExtractFileName(SingleFile);
          ofile1 := GenMakePath(ExtractRelativePath(fProject.FileName, ChangeFileExt(ofile1, OBJ1_EXT)));
        end
        else
          ofile1 := GenMakePath(ExtractRelativePath(fProject.FileName, ChangeFileExt(SingleFile, OBJ1_EXT)));
       if (devCompiler.makeName <> '') then
         cmdline:= format(cSingleFileMakeLine, [devCompiler.makeName, fMakeFile, ofile1])
       else
         cmdline:= format(cSingleFileMakeLine, [MAKE_PROGRAM, fMakeFile, ofile1]);
     end
     else begin
       if (devCompiler.makeName <> '') then
         cmdline:= format(cMakeLine, [devCompiler.makeName, fMakeFile])
    ...
    if (devCompiler.windresName <> '') then
      ...
      s := WINDRES_PROGRAM;
    cmdline := s + ' --input-format=rc -i ' + fSourceFile + ' -o ' +
               ChangeFileExt(fSourceFile, OBJ1_EXT);
    DoLogEntry(format(Lang[ID_EXECUTING], [' ' + s +cDots]));
    DoLogEntry(cmdline);
  end
  else begin
